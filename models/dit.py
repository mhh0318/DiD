...
class DIT(nn.Module, huggingface_hub.PyTorchModelHubMixin):
    def __init__(self, config):
        ...
        self.x_proj = nn.Linear(
            config.model.vae_dim * config.diffusion_category.patch_size**2,
            config.model.hidden_size,
            bias=True
        )
        self.mask_token = nn.Parameter(torch.zeros(1, 1, config.model.hidden_size))

        self.sigma_map = TimestepEmbedder(config.model.cond_dim)
        self.rotary_emb = Rotary(config.model.hidden_size // config.model.n_heads)

        self.did = config.do_did
        ...
        # )
        self.norm = nn.LayerNorm(config.model.hidden_size)
    ...
    def forward(self, x0, mask, sigma):
        x = self.x_proj(x0)
